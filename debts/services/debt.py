...
class DebtService:
    ...
    def update(debt_id, data, user=None, request=None):
        ...
        # Update fields
        update_fields = [
            'name', 'total_amount', 'paid_amount', 'due_date',
    ...
    def get_collection_schedule(period_type='monthly', as_of_date=None):
        ...
        for debt in debts:
            ...
            if amort_type == 'flat':
                ...
                if debt.interest_rate and debt.interest_rate > 0:
                    ...
                    if rate_per_period > 0:
                        ...
                        periodic_payment = (debt.total_amount * rate_per_period * factor) / (factor - 1)
                    else:
                        periodic_payment = debt.total_amount / total_periods
                else:
                    periodic_payment = debt.total_amount / total_periods
            ...
            next_due_date = debt.created_at.date() + timezone.timedelta(days=(current_period + 1) * period_info['days'])

            # Calculate total paid in period
            period_start = debt.created_at.date() + timezone.timedelta(days=current_period * period_info['days'])
            period_end = debt.created_at.date() + timezone.timedelta(days=(current_period + 1) * period_info['days'])
