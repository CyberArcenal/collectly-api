...
class LoanApplicationCreateSerializer(serializers.ModelSerializer):
    ...
    debtor_name = serializers.CharField(
        required=False,
        max_length=255,
        help_text="Debtor's full name (snapshot)"
    )
...
class LoanApplicationUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            'debtor_contact': {'required': False, 'allow_blank': True, 'allow_null': True},
            'debtor_email': {'required': False, 'allow_blank': True, 'allow_null': True},
            'debtor_address': {'required': False, 'allow_blank': True, 'allow_null': True},
...
class LoanApplicationApproveSerializer(serializers.Serializer):
    ...
    approved_at = serializers.DateTimeField(
        required=False,
        help_text="When the application was approved (defaults to now)"
    )
    ...
    def save(self, **kwargs):
        ...
        # If no debtor exists, create one from snapshot data
        if not instance.debtor:
            debtor_data = {
                'name': instance.debtor_name,
                'contact': instance.debtor_contact,
                'email': instance.debtor_email,
                'address': instance.debtor_address,
            }
            # Remove None values
            debtor_data = {k: v for k, v in debtor_data.items() if v}
            if debtor_data:
