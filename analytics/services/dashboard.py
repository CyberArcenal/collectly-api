...
class DashboardService:
    ...
    def get_overview():
        ...
        today_revenue = PaymentTransaction.objects.filter(
            deleted_at__isnull=True, payment_date__gte=today, payment_date__lt=tomorrow
        ).aggregate(total=Sum("amount"))["total"] or Decimal("0")

        # Total customers (active borrowers)
        total_customers = Borrower.objects.filter(deleted_at__isnull=True).count()
        ...
        overdue_debts = Debt.objects.filter(
            deleted_at__isnull=True, status=Debt.Status.OVERDUE
        ).count()
    ...
    def get_revenue(period="month", start_date=None, end_date=None):
        ...
        if start_date and end_date:
            qs = qs.filter(payment_date__gte=start_date, payment_date__lte=end_date)
            period_label = f"{start_date} to {end_date}"
        elif period == "today":
            today = now.date()
            tomorrow = today + timedelta(days=1)
            qs = qs.filter(payment_date__gte=today, payment_date__lt=tomorrow)
            period_label = "today"
        elif period == "week":
        ...
        stats = qs.aggregate(total=Sum("amount"), count=Count("id"))
    ...
    def get_statistics():
        ...
        total_paid = Debt.objects.filter(
            deleted_at__isnull=True, status=Debt.Status.PAID
        ).count()
    ...
    def get_top_products(limit=5):
        ...
        """
        top_debts = (
            Debt.objects.filter(deleted_at__isnull=True)
            .values("name")
        ...
        return [
            {"name": item["name"], "totalValue": float(item["total_value"] or 0)}
    ...
    def get_recent_activities(limit=10):
        ...
        audit_logs = (
            AuditLog.objects.all().select_related("user").order_by("-timestamp")[:limit]
        )
            ...
            activities.append(
                {
                    ...
                    "entityId": (
                        int(log.object_id)
                        if log.object_id and log.object_id.isdigit()
                        else None
                    ),
        ...
        # Get recent payments
        payments = (
            PaymentTransaction.objects.filter(deleted_at__isnull=True)
    ...
    def get_sales_trend(days=7):
        ...
        # payment_date is a DateField, so we can group by it directly
        payments = PaymentTransaction.objects.filter(
            deleted_at__isnull=True,
            payment_date__gte=start_date
        )

        trend = payments.values('payment_date').annotate(
            total=Sum('amount')
        ).order_by('payment_date')

        # Build a dict for quick lookup
        trend_dict = {item['payment_date']: float(item['total'] or 0) for item in trend}

        # Fill in all dates from start_date to today
        result = []
        current = start_date
    ...
    def get_payment_methods_breakdown():
        ...
        breakdown = (
            PaymentTransaction.objects.filter(
                deleted_at__isnull=True, method__isnull=False
            )
            .values("method__name")
